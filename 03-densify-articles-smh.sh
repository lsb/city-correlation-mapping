zcat pruned-{1..27}.json.gz | jq -c '{rpageid, rtime, ruser: (.runm + "/" + .ruid + "/" + .ruip)}' | pigz > revisions.json.gz
...
zcat coords-articles-*.json.gz | jq -c --slurpfile p page-densepage.json '{id: $p[0][.id], title, lat, lng}' | pigz > dense-articles.json.gz
